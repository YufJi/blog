...
借用MDN上的解释就是：块格式化上下文（Block Formatting Context） 是Web页面的可视化CSS渲染的一部分，是布局过程中生成块级盒子的区域，也是浮动元素与其他元素的交互限定区域。并且块格式化上下文包含创建它的元素内部的所有内容。
...
## BFC的作用是什么呢？
`敲重点！！！`块格式化上下文对浮动定位与清除浮动都很重要。浮动定位和清除浮动时只会应用于同一个BFC内的元素。浮动不会影响其它BFC中元素的布局，而清除浮动只能清除同一BFC中在它前面的元素的浮动。

## BFC内有哪些特殊展现呢？
👆说了BFC能很好的解决float脱离文档流带来的对其他外部BFC的影响，同时BFC自身内部也会有些特殊的展示，最明显的就是外边距折叠（margin collapsing)，下面会列出由于哪些原因会重叠哪些外边距：
